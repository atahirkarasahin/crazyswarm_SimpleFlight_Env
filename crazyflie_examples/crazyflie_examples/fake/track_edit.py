...
class FakeTrack(FakeEnv):
    def __init__(self, cfg, connection, swarm, dt=0.01):
        ...
        self.use_time_encoding = True # def False
    ...
    def _compute_state_and_obs(self) -> TensorDictBase:
        ...
        # print('rpos', self.rpos.shape)
        
        print('rpos flatten', self.rpos.flatten(1).shape)
        
        #selected_rpos = self.rpos[0:1].flatten(1)  # Keeps the batch dimension

        #print('selected_rpos', selected_rpos.shape)
        ...
        for tensor in obs:
            if tensor.shape[0] == 1:
                tensor = tensor.expand(512, -1)  # Expand batch dimension to match others

            print(tensor.shape)

        obs = [tensor.expand(512, -1) if tensor.shape[0] == 1 else tensor for tensor in obs]
        ...
        print('target_pos shape', self.target_pos[..., 0, :].shape)
        print('real_position shape', self.drone_state[..., :3].shape)
        print('drone_state shape', self.drone_state.shape)
        
        self.drone_state = self.drone_state.repeat(512, 1)  # Shape [512, 28]

        print('real_position shape repeat', self.drone_state[..., :3].shape)
        print('drone_state shape repeat', self.drone_state.shape)
    ...
    def _compute_traj(self, steps: int, env_ids=torch.tensor([0]), step_size: float=1.):
        ...
        # print(f"progress_buf shape: {self.progress_buf}")
        # print(f"arange shape: {torch.arange(steps, device=self.device).shape}")
        # print(f"traj_t0 shape: {self.traj_t0.shape}")
        # print(f"ones shape: {torch.ones(self.num_envs, 1, device=self.device).shape}")      
        ...
        t = self.traj_t0[:, None] + t[None, :] * self.dt * torch.ones(self.num_envs, steps, device=self.device)
        
        # print(f"Fixed traj_t0 shape: {self.traj_t0[:, None].shape}")
        # print(f"Fixed arange shape: {t[None, :].shape}")
        # print(f"Multiplication result shape: {(self.traj_t0[:, None] + t[None, :] * self.dt).shape}")

        # #target_pos = vmap(lemniscate_v)(t, self.T_scale[env_ids].unsqueeze(-1))
        
        # print(f"Fixed t shape: {t.squeeze(0).shape}")
        # print(f"Fixed T_scale shape: {self.T_scale[env_ids].view(-1, 1).expand(t.shape[0], t.shape[-1]).shape}")

        target_pos = vmap(lemniscate_v)(
            t.squeeze(0), 
            self.T_scale[env_ids].view(-1, 1).expand(t.shape[0], t.shape[-1])
        )

        #print(f"Fixed target_pos shape: {target_pos.shape}")
